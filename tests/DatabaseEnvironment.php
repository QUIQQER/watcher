...
final class DatabaseEnvironment
{
    ...
    public static function determineMode(array $environment): string
    {
        ...
        if (!in_array($vendor, ['mariadb', 'mysql'], true)) {
            throw new RuntimeException('GitLab watcher tests require DB_VENDOR=mysql or DB_VENDOR=mariadb.');
        }

        if (!is_string($jobId) || trim($jobId) === '') {
            throw new RuntimeException('GitLab watcher tests require CI_JOB_ID.');
        }

        if ($databaseHost !== 'db') {
            throw new RuntimeException('GitLab watcher tests require the isolated database service host "db".');
        }

        if ($databaseName !== 'quiqqer') {
            throw new RuntimeException('GitLab watcher tests require the disposable database "quiqqer".');
        }
